...
import { findPlasticRoot } from "./vcs-detect";

/** Reject path traversal / absolute paths before passing to `cm cat`. */
function validatePlasticFilePath(filePath: string): void {
...
export function createPlasticProvider(runtime: PlasticRuntime): VcsProvider {
  ...
  async function diffPending(cwd?: string): Promise<DiffResult> {
  ...
  async function diffLastChangeset(cwd?: string): Promise<DiffResult> {
    const statusOut = await runCmOrThrow(
      ["status", "--machinereadable", "--fieldseparator=|"],
      cwd,
    );
  ...
  return {
    ...
    async detect(cwd?: string): Promise<boolean> {
      return (await findPlasticRoot(cwd)) !== null;
    },

    getRoot(cwd?: string): Promise<string | null> {
      return findPlasticRoot(cwd);
    },

    ownsDiffType(diffType: string): boolean {
      return (
        diffType === "pending" ||
        diffType === "last-changeset" ||
        diffType === "plastic-branch"
      );
    },
    ...
    async getContext(cwd?: string): Promise<GitContext> {
      ...
      if (currentBranch !== defaultBranch) {
        // Namespaced id ("plastic-branch", not "branch") so it never collides
        // with Git's "branch" diff type in the shared provider registry.
        diffOptions.push({
          id: "plastic-branch",
    ...
    async getFileContents(
      ...
      validatePlasticFilePath(filePath);
    ...
    // Plastic has no staging area. stageFile/unstageFile are intentionally
    // omitted so the shared VcsApi reports canStageFiles() === false (the same
    // contract upstream's p4 provider uses).
  };
}
